...
public class CommonFileOperations {
	...
	public static void deleteIfExists(String file, FileSystem fs)
			throws IOException {
	...
	public static OutputStream openFileForWrite(String file, boolean isOnHDFS)
			throws IOException {
		if (isOnHDFS)
			return openFileForWrite(file, FileSystem.get(new Configuration()));
		else
	...
	public static OutputStream openFileForWrite(String file, FileSystem fs)
			throws IOException {
	...
	public static void copyToHDFS(String local, String hdfsFile)
			throws IOException {
	...
	public static void copyFromHDFS(String hdfsFile, String local)
			throws IOException {
		rmr(local);
	 ...
	 * @param pattern
	 *            The pattern which the file's FULL PATH matches, if it is NULL,
	 *            all file will be included (but no dir will be included)
	 * @param subdir
	 *            If it is true, then the sub directories will be searched
	 * @return The files in the directory that matches the pattern
	 * @throws IOException
	 *             Anything bad
	 */
	@SuppressWarnings("deprecation")
	public static String[] listAllFiles(String dir, String pattern,
			boolean subdir) throws IOException {
		...
		for (FileStatus f : ft) {
			if (f.isDir() && subdir) {
				String[] partial = listAllFiles(f.getPath().toString(),
						pattern, subdir);
				if (partial == null)
					continue;
	...
	public static String[] readLines(InputStream inp, String encoding)
			throws IOException {
	...
	/**
	 * Get the file size on HDFS IF the path is normal file, the size will be
	 * returned, if the path is a directory, the size of all children will be
	 * returned.
	 * 
	 * @param inputFile
	 * @return
