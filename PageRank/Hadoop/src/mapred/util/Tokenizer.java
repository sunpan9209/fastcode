...
public class Tokenizer {
	...
	public static String removePatterns(String line) {
		...
		l1 = l1.replaceAll("\\p{Punct}(\\s+|$)", " "); // All punctuations in
														// the back

		l1 = l1.replaceAll("(^|\\s+)\\p{Punct}+(\\s+|$)", " ");

		l1 = l1.replaceAll("(^|\\s+)[<>,./?!$%^&*()_+-=]", " "); // All
																	// punctuations
																	// in the
																	// front,
																	// only
		...
		l1 = l1.replaceAll("[^\\x00-\\x80]+", ""); // Remove non-ASCII chars,
													// leaving only english

		l1 = l1.replaceAll("(^|\\s+)\\d+(\\s+|$)", " "); // Remove all pure
															// digits
